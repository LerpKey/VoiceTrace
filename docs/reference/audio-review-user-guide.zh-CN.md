...
[English](audio-review-user-guide.md) · [中文](audio-review-user-guide.zh-CN.md)
...
工作台不是自动定稿工具。对于标为 `[Uncertain: …]`、`[Unclear]` 或多人同时说话的内容，
应点击句子回听原声后再用于正式文件。
...
最简单的启动方式：在项目根目录双击 `start-voicetrace-workspace.bat`。脚本会启动本机服务，并
...
录音上传后按选择执行本地/云端 ASR；勾选“使用云端增强并初次文本整理”时会一并执行一次初次 DeepSeek 文本整理。初次整理完成后，在右栏点击“总结”并输入要求，才会产生一次独立的 DeepSeek 总结请求；如果需要更换整理方式，可以点击“重新总结”再次提交。重新总结只写入 `summary_result.json` 和 `topic-index.md`，不会覆盖 `text_analysis.json`、`transcript.json`，也不会修改转写任务的完成阶段。录音进入队列后，“转写任务”区域每 2 秒自动更新并在刷新页面后继续显示，包含当前阶段、
...
`Ctrl+C`，再重新双击 `start-voicetrace-workspace.bat`。若仍出现同一提示，请保留完整启动输出用于排查。
